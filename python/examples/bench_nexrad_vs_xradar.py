"""Wall-clock benchmark: radish (engine='radish') vs xradar on a NEXRAD file.

Set ``RADISH_NEXRAD_FIXTURE`` to the path of an Archive II file before running.
...
def main():
    ...
    if not path or not os.path.exists(path):
        raise SystemExit(
            "RADISH_NEXRAD_FIXTURE must point at a NEXRAD Archive II file"
        )
    print(f"Fixture: {path}  ({os.path.getsize(path) / 1e6:.1f} MB)")
    print(f"Runs: {N}\n")

    print("xradar:")
    xradar_t = time_n(lambda: xradar.io.open_nexradlevel2_datatree(path), "xradar")
    ...
    print(f"\nSpeedup (xradar / radish): {speedup:.2f}×")
